...
describe('Progression', function () {
    test('fromRomanNumerals', function () {
        ...
        expect($inC(explode(' ', 'I IIm7 V7')))->toBe(explode(' ', 'C Dm7 G7'))
            ->and($inC(explode(' ', 'Imaj7 2 IIIm7')))->toBe(['Cmaj7', '', 'Em7'])
            ->and($inC(explode(' ', 'I II III IV V VI VII')))->toBe(explode(' ', 'C D E F G A B'))
            ->and($inC(explode(' ', 'bI bII bIII bIV bV bVI bVII')))->toBe(explode(' ', 'Cb Db Eb Fb Gb Ab Bb'))
            ->and($inC(explode(' ', '#Im7 #IIm7 #III #IVMaj7 #V7 #VI #VIIo')))
    ...
    test('fromRomanNumerals with different tonics', function () {
        expect(Progression::fromRomanNumerals('G', ['I', 'IV', 'V']))
            ->toBe(['G', 'C', 'D'])
            ->and(Progression::fromRomanNumerals('F', ['Im7', 'IVm7', 'V7']))
            ->toBe(['Fm7', 'Bbm7', 'C7']);
    ...
    test('toRomanNumerals with different tonics', function () {
        expect(Progression::toRomanNumerals('G', ['G', 'C', 'D']))
            ->toBe(['I', 'IV', 'V'])
            ->and(Progression::toRomanNumerals('A', ['A', 'D', 'E7']))
            ->toBe(['I', 'IV', 'V7']);
